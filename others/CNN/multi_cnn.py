...
model.add(Conv2D(15,(3,3),activation="relu")) #中间卷积层
model.add(MaxPooling2D(pool_size=(2,2)))#池化层
...
model.add(Dense(units=50,activation="relu"))#中间层
model.add(Dense(units=10,activation="softmax"))#输出层
model.compile(optimizer="adam",loss="categorical_crossentropy",metrics=['accuracy'])
model.fit(X_train,y_train,epochs=10,batch_size=200,verbose=2) #训练
score=model.evaluate(X_validation,y_validation)
print("CNN Big %f" %(score[1]))
